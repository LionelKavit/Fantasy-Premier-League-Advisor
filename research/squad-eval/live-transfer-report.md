# Transfer live-eval — full pipeline, 2026-27 (PROVISIONAL, n=2)
...
**Provisional:** 2 decision gameweeks scored — read directionally, not conclusively.

### Decision points: 2 gameweeks
- App recommended a transfer in **2/2** GWs (held 0); you transferred in **2/2**.
- Realized so far: next-1 scored for **2/2**, next-3 for **0/2** (the rest span gameweeks still being played).

**Counterfactual gain vs holding** (realized in − out):
| | next-1 GW | next-3 GW |
|---|---|---|
| App recommendation | 3.50 | unavailable — no scored GWs |
| Your actual transfers (net of hits) | 0.00 | unavailable — no scored GWs |
...
| 5 | Cash→Bogle | pending | transfer | pending |

## Full pipeline vs deterministic floor

| | 2025-26 replay floor (ep_next absent, 35 decision GWs, 1 FT assumed) | Live full pipeline (n=2, provisional) |
|---|---|---|
| App transfer rate | 0/35 (held every GW) | 2/2 |
| App mean gain next-1 / next-3 | 0.00 / 0.00 | 3.50 / — |
| Your mean gain next-1 / next-3 (net) | 3.49 / 5.83 | 0.00 / — |
...
| 5 | 1 | £0.2m | FREE: Cash→Bogle | +7.4 | +6 | 0 | Maguire→Bogle | 0 |
| 6 | 1 | £0.5m | FREE: Cash→De Cuyper | +5.0 | pending | 0 | pending | — |

**Calibration (n=2):** mean projected Δep 7.05 vs mean realized next-1 (in − out, net of hit) 3.50.
