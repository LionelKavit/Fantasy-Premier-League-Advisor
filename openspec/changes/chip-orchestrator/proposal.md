...
- **A chip orchestrator** (new synthesis). Single LLM call: system grounding = `chips.md`; input = the deterministic candidate windows + chip state + fixture facts (built deterministically — the orchestrator may only reason over provided facts, never invent gameweeks/Doubles). Output = a **structured** plan: for each remaining chip, promote a `window` to `play-now` (this gameweek), sequence it for a future gameweek, or hold — each with reasoning and confidence.
