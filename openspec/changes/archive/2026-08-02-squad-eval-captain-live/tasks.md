...
> **Audit + first scoring (2026-09-04):** verified implemented and running — `live-log.json` held real pre-deadline captures for GW2 (2026-08-28 17:25Z, deadline 17:30Z) and GW3 (16:46Z today, via the launchd brief's safety-net capture), full pipeline with LLM. Gaps closed: (1) `score-live.ts` had never been run, so no `live-report.md` existed; (2) scoring compared against the squad the capture saw, which is structurally the **previous** GW's locked squad (the public API exposes nothing newer pre-deadline) — for GW2 that squad had Cash/Ajer in the XI and Haaland/Sangaré out, and its "actual captain" is the G-1 armband, which would have mis-scored GW3 (armband moved João Pedro → Haaland before lock). Scoring now fetches the **actually-locked** picks post-deadline into `realized` (XI/bench with points, captain/vice, chip, GW points/rank, transfers + hit) and reports any drift; (3) GW1 has no capture (impossible pre-deadline) — `score-live.ts` now backfills a picks-only `captureMode: "retrospective"` record so the season log is complete from GW1 (never scored). First result, n=1: GW2 app pick B.Fernandes 23 pts = best in XI (hit), vs the manager's João Pedro 9 → +14 captain-pts. Provisional.
