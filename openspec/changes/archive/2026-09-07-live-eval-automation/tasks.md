...
> **Addendum (2026-09-18):** the tick-written artefacts (`live-log.json`, `live-report.md`, `live-transfer-report.md`, `live-dataset.*`, `live-pool-dataset.*`, `pool/`) were still tracked on code branches, so every tick left the checkout dirty and `git checkout`/`git pull` refused (hit on 2026-09-07 for `live-status.md` and again on 2026-09-18 for the rest). They are now gitignored on code branches and untracked (working copies kept); the `live-eval-data` branch remains the sole history. A fresh checkout starts with no `live-log.json` — the first capture creates it.
